    ...
    return (
        <div className="mt-25 px-7 flex gap-5 flex-wrap">
            {projects.map((val, index) =>
                <Link to={`/video/${index}`} key={index} className={`h-[420px] w-[310px] bg-[#1A1A1A] rounded-xl space-y-3 relative ease-in-out duration-[1s] opacity-0 ${visible ? 'opacity-100 translate-y-[-40px]' : ''}`} style={{ transitionDelay: `${index * 100}ms` }} onMouseOver={() => setHover(index)} onMouseLeave={() => setHover(-1)}>
                    ...
                    <div className="px-4 text-lg text-red-700 font-semibold">{val.name}</div>
                    ...
                    <div className="flex pl-4 text-sm gap-2 flex-wrap">
                        {val.language.map((value, ind) => {
                            return (
                                <div
                                    key={ind}
                                    className="bg-gradient-to-br from-gray-800 to-gray-900 border border-gray-700 px-2 py-1 rounded-lg flex gap-2 items-center hover:border-red-500 hover:shadow-lg hover:shadow-red-500/20 transition-all duration-300 cursor-pointer group"
                                >
