    ...
    return(
        ...
        <div className='flex gap-x-2 md:gap-x-30'>
          ...
          <div className={`fixed top-0 left-0 z-20 md:hidden flex flex-col space-y-10 bg-black/70 h-[100vh] w-[50vw] pt-15 pl-10 ${nav ? '' : 'translate-x-[-50vw]'} duration-500 ease-in-out`}>
            <Link to={'/'} onClick={()=>setNav(false)} className='hover:text-red-700 pt-10 cursor-pointer ease-in-out duration-[0.5s]'>Home</Link>
            <Link to={'/professional'} onClick={()=>setNav(false)} className='hover:text-red-700 cursor-pointer ease-in-out duration-[0.5s]'>Professional</Link>
            <Link to={'/skills'} onClick={()=>setNav(false)} className='hover:text-red-700 cursor-pointer ease-in-out duration-[0.5s]'>Skills</Link>
            <Link to={'/projects'} onClick={()=>setNav(false)} className='hover:text-red-700 cursor-pointer ease-in-out duration-[0.5s]'>Projects</Link>
            <div className='hover:text-red-700 cursor-pointer ease-in-out duration-[0.5s]'>Videos</div>
          </div>
        </div>
        <div className='h-10 w-10 bg-[#1E1E1E] rounded-full overflow-hidden flex items-center justify-center'>
